...
exports.plans = function (req, res) {
	...
	var year = req.params.year,
		plans = [],
		q = dev.checkQuery(
			`SELECT planname, featuretyp
			FROM (
				SELECT planyear::int, planname, featuretyp FROM plannedpoly
				UNION SELECT planyear::int, planname, featuretyp FROM plannedline
				ORDER BY planyear, planname, featuretyp
			) AS q WHERE planyear = $1`, req);

	client.query(q, [year], function (err, plans) {
		res.send(plans.rows);
		client.end();
	});
}
...
exports.collector = function (req, res) {
	...
	var q = "INSERT INTO viewsheds_dev ( layer, globalid, creator, repository, firstdispl, lastdispla, imageid, title, geom, uploaddate, latitude, longitude ) VALUES ( 'viewsheds', " + data.globalid + ", '" + data.creator + "', '" + data.repository + "', " + data.firstdispl + ", " + data.lastdispla + ", '" + data.imageid + "', '" + data.title + "', ST_SetSRID(ST_GeomFromGeoJSON('" + JSON.stringify(geo.geometry) + "'), 4326), 9999, " + data.latitude + ", " + data.longitude + ")";
