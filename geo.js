...
exports.box = function( req, res ){
	...
	var year = req.params.year,
			...
			layers = req.params.layers,
			results = [],
			q = dev.checkQuery( "SELECT array_agg( id ) AS id, name, array_agg( file ) AS file, layer FROM ( SELECT globalid AS id, namecomple AS name, layer, NULL AS file, geom FROM baseline WHERE namecomple IS NOT NULL AND firstdispl <= " + year + " AND lastdispla >= " + year + " UNION SELECT globalid AS id, namecomple AS name, layer, NULL AS file, geom FROM basepoly WHERE namecomple IS NOT NULL AND firstdispl <= " + year + " AND lastdispla >= " + year + " UNION SELECT globalid AS id, namecomple AS name, layer, NULL AS file, geom FROM basepoint WHERE namecomple IS NOT NULL AND firstdispl <= " + year + " AND lastdispla >= " + year + " UNION SELECT imageid AS ID, title AS name, 'SSID' || globalid AS file, layer FROM viewsheds WHERE firstdispl <= " + year + " AND lastdispla >= " + year + " ORDER BY layer ) as q WHERE geom && ST_MakeEnvelope( " + c1 + ", " + c2 + ", 4326 ) GROUP BY name, layer ORDER BY layer", req );
